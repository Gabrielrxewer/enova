...
def rep_os():
    ...
    def browse():
        with sqlite3.connect("C:\\Users\\inspe\\Desktop\\Qualidade\\Projetos py\\os.db") as conn:
            c = conn.cursor()
            c.execute("SELECT * FROM tb_OS WHERE COD_OS=?", (eos_list[0].get(),))
            global res
            res = c.fetchone()
            if res:
                for i in range(1, 7):
    ...
    def save_data():
        ...
        global res2
        desc=['cod_os', 'desc_os', 'resp_os', 'tipo_os', 'equip_os', 'setor_os', 'infos_os', 'inic_os', 'term_os', 'exec_os']
        for i in eos_list:
    ...
    labels = ["Código da O.S", "Descrição da O.S", "Responsável pela O.S", "Tipo da O.S", "Equipamento", "Setor", "Informações", "Horário de Início", "Horário de Término", "Executor"]
    eos_list = []

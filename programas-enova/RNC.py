...
def adicionar_rnc():
    # Obter a data atual
    data_atual = '05/04/2023'
    # Atualizar a contagem de RNC's para a data atual
    if data_atual in rnc_por_dia:
        rnc_por_dia[data_atual] += 1
        cursor.execute('UPDATE rnc_por_dia SET contagem = ? WHERE data = ?',
                       (rnc_por_dia[data_atual], data_atual))
    else:
        rnc_por_dia[data_atual] = 1
        cursor.execute(
            'INSERT INTO rnc_por_dia (data, contagem) VALUES (?, ?)', (data_atual, 1))
    conn.commit()
    atualizar_grafico()

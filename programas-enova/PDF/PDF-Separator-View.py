...
def abrir_pdf():
    ...
    filename = askopenfilename(defaultextension=".pdf", filetypes=[("Arquivo PDF", "*.pdf")])
    if not filename:
        return
...
def mostrar_imagem():
    ...
    imagem_canvas.create_image(0, 0, anchor=NW, image=imagem_labels[pagina_atual].image)
...
root.bind_all("<MouseWheel>", lambda event: imagem_canvas.yview_scroll(int(-1*(event.delta/120)), "units"))
...
btn_proxima_pagina = Button(footer_frame, text="Próxima Página", command=proxima_pagina)
btn_proxima_pagina.pack(side="bottom", padx=10)

# cria o botão para voltar a página
btn_proxima_pagina = Button(footer_frame, text="Voltar Página", command=voltar_pagina)
btn_proxima_pagina.pack(side="bottom", padx=10)
